...
void *ecatthread(void *ptr) {
    ...
    auto data_recorder =
        std::make_shared<edm::util::DataQueueRecorder<Data>>("output.bin");

    int policy = -1;
    sched_param param;
    pthread_getschedparam(pthread_self(), &policy, &param);
    s_logger->info("*** thread policy: {}, pri: {}", (policy), (int)param.sched_priority);
    ...
    while (1) {
        ...
        int diff = curr_ts.tv_nsec - (last_curr_ts.tv_nsec + next_cycle_time);
        ...
        if (dorun > 0) {
            ...
            if (record_count <= 5000) {
                // Data data{d->get_actual_position(), target_pos};
                // data_recorder->push_data(data);
