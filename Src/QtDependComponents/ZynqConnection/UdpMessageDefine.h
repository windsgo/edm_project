...
struct udp_recv_servo_msg {
    struct udp_adc_data realtime_adc_data; // 获取数据时PL端实时采集到的瞬时电压
    struct udp_adc_data pl_filtered_adc_data; // PL端(ns~us级别)滤波的电压
    struct udp_adc_data ps_filtered_adc_data; // PS端(宏观ms级)滤波的电压
};
